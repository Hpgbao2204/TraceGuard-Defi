...
def _packet_identity_errors(rows: list[dict], packet_key: str,
                           expected_fixed_hash: str,
                           packet_manifest: dict | None) -> list[str]:
    """Reject submissions that are not derived from the frozen packet."""
    if not rows:
        return []
    errors: list[str] = []
    expected_packet_hash = ((packet_manifest or {}).get("packet_hashes") or {}).get(packet_key)
    for row in rows:
        if row.get("packet_schema_version") != 1:
            errors.append("packet_schema_version_missing_or_invalid")
        if row.get("fixed_set_sha256") != expected_fixed_hash:
            errors.append("fixed_set_sha256_mismatch")
        if not isinstance(row.get("packet_sha256"), str):
            errors.append("packet_sha256_missing")
        elif expected_packet_hash and row["packet_sha256"] != expected_packet_hash:
...
def audit_e4(first: list[dict], second: list[dict], fixed: dict,
    ...
    fixed_hash = fixed_hash or sha256_file(E4_FIXED)
    packet_manifest = None
    if PACKET_MANIFEST.is_file():
        packet_manifest = json.loads(PACKET_MANIFEST.read_text(encoding="utf-8"))
    result["errors"].extend(_packet_identity_errors(
        first, "e4_reviewer_a", fixed_hash, packet_manifest))
    result["errors"].extend(_packet_identity_errors(
        second, "e4_reviewer_b", fixed_hash, packet_manifest))
    result["ready_for_adjudication"] = not result["errors"] and result["reviewed_complete"] == result["expected"]
    return result
...
def audit_e4_files(first_path: Path, second_path: Path, fixed_path: Path,
    ...
    return audit_e4(
        ...
        _load_jsonl(final_path) if final_path.is_file() else None,
...
def audit_hard(first: list[dict], second: list[dict], queue: list[dict],
    ...
    queue_hash = queue_hash or sha256_file(HARD_QUEUE)
    packet_manifest = None
    if PACKET_MANIFEST.is_file():
        packet_manifest = json.loads(PACKET_MANIFEST.read_text(encoding="utf-8"))
    result["errors"].extend(_packet_identity_errors(
        first, "hard_negatives_reviewer_a", queue_hash, packet_manifest))
    result["errors"].extend(_packet_identity_errors(
        second, "hard_negatives_reviewer_b", queue_hash, packet_manifest))
    result["ready_for_adjudication"] = not result["errors"] and result["reviewed_complete"] == result["expected"]
    return result
