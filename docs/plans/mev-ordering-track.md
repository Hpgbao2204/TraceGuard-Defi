...
7. **Layer 2 chạy trên engine của bài** (`--l2-engine geth`, `eval/mev_sim/geth_bridge.py`): mỗi bundle bị layer 1 gắn cờ được đào thành một block anvil, xuất thành context B2 có proof EIP-1186, rồi `geth-replay -chain-id 31337 -lean` chạy hai lần: baseline phải qua `acceptance_gate` (gate fidelity RQ2), sau đó `-drop-tx` (M1) ra verdict. Số RQ4 trong bài lấy từ chế độ này. Layer 1 là bộ lọc mức bundle, **không phải** screener 3 view của RQ1; bài phải nói rõ như vậy.
