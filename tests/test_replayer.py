...
class RpcClientTests(unittest.TestCase):
    ...
    def test_network_diagnostics_redact_endpoint_credentials(self):
        endpoint = "https://eth.example/v2/super-secret-api-key"
        with mock.patch("urllib.request.urlopen", side_effect=OSError("no network")):
            with self.assertRaises(RpcError) as raised:
                RpcClient(endpoint).call("eth_blockNumber")
        self.assertNotIn("super-secret-api-key", str(raised.exception))
        self.assertIn("https://eth.example/<redacted>", str(raised.exception))
