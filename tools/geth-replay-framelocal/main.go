	...
	for i, tx := range txs[:*targetIndex+1] {
		...
		if i == *targetIndex {
			...
			if *mode == "frame-local" || *mode == "isolation" || *mode == "sham" {
				...
				isSham := *mode == "sham"
				scopingMgr := newScopingManager(enableScoping, victims, priceSources, s0Snapshot, &header, chainConfig, lambdaPtr, isSham)
				...
				scopingMgr := newScopingManager(*scopedPrice, victims, priceSources, s0Snapshot, &header, chainConfig, lambdaPtr, false)
