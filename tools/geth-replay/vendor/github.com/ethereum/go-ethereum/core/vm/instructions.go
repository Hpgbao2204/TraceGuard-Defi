...
func opCall(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	if evm.CallIntervention != nil {
		handled, rewritten, synthetic, interventionErr := evm.CallIntervention(byte(CALL), evm.depth, scope.Contract.Address(), toAddr, args, childBudget, &value)
		if rewritten != nil {
			args = rewritten
		}
		if handled {
			// A synthetic intervention still owns the forwarded child budget.
			// Return its unused gas exactly as a successful zero-side-effect
			// child call would; otherwise substitute interventions artificially
			// consume the entire child budget and can create false OOGs.
			scope.Contract.refundGas(childBudget.ExitSuccess(), evm.Config.Tracer, tracing.GasChangeCallLeftOverRefunded)
			...
			stack.push(&temp)
			if interventionErr == nil || interventionErr == ErrExecutionReverted {
				scope.Memory.Set(retOffset.Uint64(), retSize.Uint64(), synthetic)
			}
			evm.returnData = synthetic
			return synthetic, interventionErr
		}
	}
...
func opDelegateCall(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	childBudget := NewGasBudget(gas, scope.Contract.Gas.StateGas)
	if evm.CallIntervention != nil {
		handled, rewritten, synthetic, interventionErr := evm.CallIntervention(byte(DELEGATECALL), evm.depth, scope.Contract.Address(), toAddr, args, childBudget, nil)
		if rewritten != nil {
			args = rewritten
		}
		if handled {
			// Preserve forwarded gas for synthetic STATICCALL interventions.
			scope.Contract.refundGas(childBudget.ExitSuccess(), evm.Config.Tracer, tracing.GasChangeCallLeftOverRefunded)
			if interventionErr != nil {
				temp.Clear()
			} else {
...
func opStaticCall(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	childBudget := NewGasBudget(gas, scope.Contract.Gas.StateGas)
	if evm.CallIntervention != nil {
		handled, rewritten, synthetic, interventionErr := evm.CallIntervention(byte(STATICCALL), evm.depth, scope.Contract.Address(), toAddr, args, childBudget, nil)
		if rewritten != nil {
			args = rewritten
		}
		if handled {
