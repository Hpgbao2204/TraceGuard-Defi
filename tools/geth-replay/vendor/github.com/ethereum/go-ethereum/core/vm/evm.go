...
type (
	...
	// CallIntervention is invoked immediately before child-call dispatch. The
	// callback must return handled=false for non-matching frames. A handled
	// frame supplies synthetic output/error and bypasses canonical dispatch.
	CallIntervention func(op byte, depth int, from, to common.Address, input []byte, budget GasBudget, value *uint256.Int) (handled bool, rewrittenInput []byte, output []byte, err error)
)
...
type EVM struct {
	...
	// CallIntervention is nil for canonical replay. It is intentionally an
	// injection seam, not a generic state or bytecode override.
	CallIntervention CallIntervention
