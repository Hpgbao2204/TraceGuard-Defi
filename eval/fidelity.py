...
class E5Replayer(Replayer):
    ...
    def replay_same_block(self, prior_hashes: list[str], target_hash: str,
                          ...
                          warmup_deadline_seconds: float = 120.0) -> ReplayResult:
        ...
        if warmup_deadline_seconds <= 0:
            raise ValueError("warm-up deadline must be positive")
        deadline = time.monotonic() + warmup_deadline_seconds
        queued: list[tuple[str, str]] = []
        local = _local_client(self.fork.url, self.timeout)
        topups: dict[str, int] = {}
        for index, h in enumerate([*prior_hashes, target_hash]):
            if index < len(prior_hashes) and time.monotonic() >= deadline:
                self.warmup_failures.append({
                    "tx_hash": h, "observed": False, "status": None,
                    "reason": f"warm-up deadline exceeded ({warmup_deadline_seconds:.1f}s)",
                })
                return ReplayResult(Outcome.UNOBSERVED, observed=False,
                                    error_kind="warmup_timeout",
                                    note="batch prefix warm-up deadline exceeded")
            try:
                tx = self.tx_parts(h)
            except RpcError as exc:
