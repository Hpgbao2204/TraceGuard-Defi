...
from core.env import load_dotenv, resolve_rpc, resolve_rpc_candidates, resolve_trace_rpc_candidates
...
from .e4.pricing import load_price_manifest, pool_harm_spec_from_manifest
...
def _legacy_main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--case-timeout", type=int, default=300,
                        help="Whole-case deadline covering acquisition, planning, and replay")
    ...
    if args.paper_fixed20 and out_path.exists() and not args.resume:
    ...
    # Paper runs must remain case-fault tolerant: one archive endpoint stall
    # must not consume the whole run.  Provider fallback is still enabled,
    # but each endpoint gets one bounded attempt; the outer case handler
    # persists INCONCLUSIVE-transport and advances when all endpoints fail.
    archive = RpcClient(rpc, timeout=args.timeout, attempts=1,
                        fallback_urls=resolve_rpc_candidates("mainnet")[1:])
    price_manifest = load_price_manifest(REPO_ROOT / "eval" / "e4_price_manifest.json")
    ...
    trace_client = RpcClient(trace_url, timeout=args.timeout, attempts=1,
    ...
    manifest_parameters = {
        ...
        "fixed_attempted_cases": fixed_case_order,
    ...
    if args.resume:
        ...
        for name, path in input_paths.items():
            expected = previous.get("inputs", {}).get(name, {}).get("sha256")
            actual = __import__("hashlib").sha256(Path(path).read_bytes()).hexdigest()
            if expected != actual:
                parser.error(f"--resume input hash mismatch: {name}")
        previous_params = previous.get("parameters", {})
        if previous_params.get("fixed_attempted_cases") != fixed_case_order:
            parser.error("--resume fixed case order mismatch")
        for key in ("paper_fixed20", "sham_control", "joint_pairs", "max_warmup_tx", "case_timeout_s"):
            if previous_params.get(key) != manifest_parameters[key]:
                parser.error(f"--resume parameter mismatch: {key}")
        print(f"resume manifest verified: {manifest_path}")
    else:
        write_manifest(
            manifest_path, run_id=run_id, experiment="E4-counterfactual-necessity-v2",
            repository=REPO_ROOT, inputs=input_paths, parameters=manifest_parameters,
    ...
    for i, j in enumerate(todo, 1):
        ...
        case_deadline = time.monotonic() + args.case_timeout
        archive.timeout = min(args.timeout, args.case_timeout)
        trace_client.timeout = min(args.timeout, args.case_timeout)
        try:
            case = (_pilot_case(case_id, archive, trace_cache, args.trace_mode)
                    or _row_from_corpus(j, archive, trace_cache, args.trace_mode))
            if case_id == "pilot-bzx" and case.block is not None:
                legacy_spec = case.extra.get("harm_spec") or {}
                case.extra["harm_spec"] = pool_harm_spec_from_manifest(
                    archive, price_manifest, case.block - 1,
                    legacy_spec.get("protected_owner", ""),
                    target_block=case.block,
                    protected_asset=legacy_spec.get("protected_asset", "native eth"),
                    protected_token=legacy_spec.get("protected_token"),
                )
            ...
            if case.trace is None and args.trace_mode != "cache-only":
                if time.monotonic() >= case_deadline:
                    raise RpcError("planner-timeout: case deadline exceeded before trace acquisition")
                case.trace = _resolve_trace(trace_client, case.tx_hash)
            if time.monotonic() >= case_deadline:
                raise RpcError("planner-timeout: case deadline exceeded before mutation planning")
