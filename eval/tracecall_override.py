...
def _call_object(tx: dict) -> dict:
    ...
    for key in ("gasPrice", "maxFeePerGas", "maxPriorityFeePerGas"):
        if tx.get(key) is not None:
            out[key] = tx[key]
    # Some RPC responses expose legacy and EIP-1559 fields together.  The
    # call object must never send both fee envelopes to QuickNode/Geth.
    if out.get("maxFeePerGas") is not None or out.get("maxPriorityFeePerGas") is not None:
...
def trace_call(client: RpcClient, tx: dict, block: int,
               ...
               shape: str = "geth", tx_index: int | None = None) -> TraceCallResult:
...
def run_probe(rpc: str, tx_hash: str, block: int, override: dict | None = None,
              output: Path | None = None, tx_index: int | None = None) -> dict:
    ...
    baseline = trace_call(client, tx, block, tx_index=tx_index)
    results = [baseline]
    if override is not None:
        # Geth/QuickNode-compatible shape: stateOverrides is inside the
        # third TraceCallConfig parameter, never a fourth positional arg.
        results.append(trace_call(client, tx, block, override, "nested", tx_index))
    payload = {
        "tx_hash": tx_hash,
        "block": block,
        "mainnet_receipt": {
...
def main() -> int:
    ...
    parser.add_argument("--patched-artifact", default=None,
                        help="Foundry artifact containing deployedBytecode.object")
    parser.add_argument("--tx-index", type=int, default=None)
    ...
    if args.patched_artifact:
        artifact = json.loads(Path(args.patched_artifact).read_text(encoding="utf-8"))
        code = artifact.get("deployedBytecode", {}).get("object")
        if not code or code == "0x":
            parser.error("patched artifact has no deployedBytecode.object")
        override = {"0xf3f84ce038442ae4c4dcb6a8ca8bacd7f28c9bde": {"code": code}}
    ...
    payload = run_probe(rpc, args.tx, args.block, override,
                        Path(args.output) if args.output else None, args.tx_index)
