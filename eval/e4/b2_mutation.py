...
class OracleStubProvider:
    """Build a selector-scoped oracle override with a shadow implementation."""

    SHADOW_ADDRESS = "0x000000000000000000000000000000000000f1a3"

    @staticmethod
    def _require_absent_shadow(context: str | Path, address: str) -> None:
        try:
            artifact = json.loads((Path(context) / "prestate_proofs.json").read_text())
            item = next(item for item in artifact.get("proofs", [])
                        if str(item.get("address", "")).lower() == address.lower())
            code_hash = str((item.get("proof") or {}).get("codeHash", ""))
        except (OSError, StopIteration, TypeError, json.JSONDecodeError) as exc:
            raise ValueError("f_orc shadow absence proof is missing") from exc
        if code_hash.lower() not in {"0x0", "0x" + "0" * 64}:
            raise ValueError("f_orc shadow address is not proven absent at state block")

    def override(self, mutation: OraclePin) -> dict[str, dict[str, str]]:
        """Legacy pure translation retained for callers without context."""
    ...
    def override_for_context(self, context: str | Path, mutation: OraclePin) -> dict[str, dict[str, str]]:
        if not mutation.selector:
            raise ValueError("f_orc requires the getter selector observed by callTracer")
        if not mutation.expected_return or not mutation.reference_verified:
            raise ValueError("f_orc requires a verified historical reference return")
        try:
            case = json.loads((Path(context) / "case.json").read_text())
            target_block = int(case["block"])
            expected_reference_block = int(case.get("state_block", target_block - 1))
        except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ValueError("f_orc context is missing a valid target/state block") from exc
        if mutation.reference_block != expected_reference_block:
            raise ValueError("f_orc reference block does not match context state block")
        self._require_absent_shadow(context, self.SHADOW_ADDRESS)
        accounts = _load_prestate_accounts(context)
        original = next((value.get("code") for address, value in accounts.items()
                         if str(address).lower() == mutation.oracle.lower()), None)
        if not original or original in ("0x", "0x0"):
            raise ValueError("f_orc oracle code missing from prepared prestate")
        runtime = _oracle_forwarding_runtime(
            self.SHADOW_ADDRESS, mutation.selector, mutation.expected_return)
        return {"target_code": {
            mutation.oracle: runtime,
        }, "target_code_copy": {self.SHADOW_ADDRESS: mutation.oracle}}
...
def mutation_args(context: str | Path, mutation: Mutation) -> tuple[dict, str | None]:
    ...
    if isinstance(mutation, CompositeMutation):
        merged: dict = {"target_code": {}, "target_code_copy": {}, "target_storage": {}}
        for child in mutation.mutations:
            ...
            merged["target_code_copy"].update(args.get("target_code_copy", {}))
    ...
    if isinstance(mutation, OraclePin):
        try:
            return OracleStubProvider().override_for_context(context, mutation), None
    ...
    if isinstance(mutation, StorageSlotWrite):
        return {"target_storage": {mutation.address: {
            mutation.slot: mutation.value,
        }}}, None
    ...
    if isinstance(mutation, FlashLoanDisable):
        try:
            ...
            OracleStubProvider._require_absent_shadow(context, mutation.SHADOW_ADDRESS)
            guard = _operate_guard_runtime(mutation.SHADOW_ADDRESS, mutation.selector)
            return {"target_code": {
                mutation.provider: guard,
            }, "target_code_copy": {mutation.SHADOW_ADDRESS: mutation.provider}}, None
        except (OSError, TypeError, ValueError, json.JSONDecodeError) as exc:
