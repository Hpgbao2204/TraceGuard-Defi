...
from eval.e4.policy_v2 import OFFICIAL_M2_OPERATORS
from eval.e4.semantic_validation import flash_loan_postcondition, oracle_postcondition
...
def run_necessity(case: Case, mutations: list[Mutation] | None = None,
    ...
    harm_spec_id = ""
    if isinstance(harm_spec, dict):
        declared_id = str(harm_spec.get("harm_spec_id") or "")
        if declared_id:
            harm_spec_id = declared_id
        else:
            harm_spec_id = n.hashlib.sha256(
                json.dumps(harm_spec, sort_keys=True, separators=(",", ":")).encode()
            ).hexdigest()[:16]
    ...
    if b2_context and isinstance(harm_spec, dict):
        if harm_spec.get("oracle", "").endswith("_transfer_delta"):
            ...
            baseline_harm = _assess_attacker_value_harm(
                ...
                lmin_usd=harm_spec.get("lmin_usd", 100_000.0),
            )
    ...
    rows.append({
        ...
        "harm_spec_id": harm_spec_id,
        "harm_source": baseline_harm.source,
        "harm_source_mutated": "",
    ...
    for m in planned_mutations:
        if b2_context:
            ...
            if unsupported:
                rows.append({
                    ...
                    "mutation_application_verified": False,
                    "mutation_contract_verified": False,
                    "official_m2_operator": mutation_factor(m_name) in OFFICIAL_M2_OPERATORS,
            ...
            official_operator = mutation_factor(m_name) in OFFICIAL_M2_OPERATORS
            application_verified, application_reason = application_gate(
                mutated.payload, b2_args, case.tx_index)
            semantic_status, semantic_reason = "unknown", "semantic-contract-unsupported"
            if isinstance(m, n.OraclePin):
                semantic_status, semantic_reason = oracle_postcondition(
                    mutated.payload,
                    oracle=m.oracle,
                    selector=m.selector or "",
                    expected_return=m.expected_return,
                    reference_verified=m.reference_verified,
                )
            elif isinstance(m, n.FlashLoanDisable):
                semantic_status, semantic_reason = flash_loan_postcondition(
                    mutated.payload, provider=m.provider, selector=m.selector)
            ...
            intervention_valid = (official_operator and execution_preserving
                                  and application_verified and semantic_valid)
            ...
            if not official_operator:
                verdict = "INCONCLUSIVE-operator-out-of-scope"
            elif baseline_status == "UNKNOWN":
                ...
                verdict = f"INCONCLUSIVE-application-{application_reason}"
            elif not semantic_valid:
                verdict = f"INCONCLUSIVE-semantic-{semantic_reason}"
                ...
                verdict = evaluate_removal_intervention(
                    ...
                    intervention_supported=intervention_valid,
                )
            validity_reason = (
                "execution_preserving_application_and_semantic_verified"
                if intervention_valid else
                mutated.error if not execution_preserving else
                application_reason if not application_verified else semantic_reason
            )
            rows.append({
                "run_id": run_id, "planner": "blind-v2", "case": case.case_id,
                "mutation": m_name, "candidate_factor": mutation_factor(m_name),
                ...
                "mutation_application_verified": application_verified,
                "mutation_contract_verified": semantic_valid,
                # Diagnostic compatibility alias; policy uses the canonical
                # mutation_contract_verified field above.
                "semantic_valid": semantic_valid,
                "official_m2_operator": official_operator,
                "semantic_reason": semantic_reason,
                "validity_reason": validity_reason if execution_preserving else mutated.error,
                ...
                "harm_spec_id": harm_spec_id,
                "harm_source": baseline_harm.source,
                "harm_source_mutated": mutated_harm.source,
                ...
                "note": (f"B2 mutation runner; target_index={mutated.payload.get('target_index')}; "
                         ...
                         f"application={application_reason}; semantic={semantic_reason}; "
                         f"harm={mutated_harm.reason}; "
                         f"error={mutated.error or 'none'}"),
            })
            continue
        ...
        if calldata_only:
            ...
            with ForkRunner(rpc or resolve_rpc(), state_block, port=port,
                ...
                if case.prior_hashes:
                    warmup_failed = rp.warmup(case.prior_hashes, deadline_seconds=120.0)
                    if warmup_failed:
                        raise RpcError(
                            f"warm-up deadline/transport failure: {len(warmup_failed)} "
                            f"prefix transaction(s) unavailable"
                        )
                m.apply_to_replayer(rp)
                m.apply(fork)
        ...
        elif not official_operator:
            verdict = "INCONCLUSIVE-operator-out-of-scope"
            intervention_valid = False
            validity_reason = "operator-out-of-scope"
        else:
            verdict = criterion(
                mutated_outcome,
        ...
        rows.append({
            ...
            "intervention_valid": intervention_valid,
            "mutation_application_verified": intervention_valid,
            "mutation_contract_verified": intervention_valid,
            "official_m2_operator": official_operator,
