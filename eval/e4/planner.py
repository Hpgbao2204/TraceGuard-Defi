...
def _historical_oracle_return(archive: RpcClient, oracle: str, block: int,
                              selector: str) -> str | None:
    """Acquire the exact getter return used as the f_orc reference.

    The result is retained byte-for-byte.  Decoding a price from a tuple is
    useful for diagnostics, but is not sufficient to define a semantic
    intervention because tuple metadata must be preserved too.
    """
    try:
        result = archive.call(
            "eth_call", [{"to": oracle, "data": "0x" + selector}, _block_tag(block)])
    except RpcError:
        return None
...
def build_mutation_plan(case: Case, archive: RpcClient | None = None,
    ...
    reference_return = (
        _historical_oracle_return(archive, oracle, block, oracle_selector)
        if oracle and oracle_selector else None
    )
    # The exact raw eth_call return is the semantic authority.  The stub's
    # immutable is only a compatibility value for non-target selectors; derive
    # it from the observed return rather than making a decoded scalar price the
    # acceptance condition for f_orc.
    price = None
    if reference_return:
        body = reference_return[2:]
        if oracle_selector == "feaf968c" and len(body) >= 128:
            price = int(body[64:128], 16)
        elif len(body) >= 64:
            price = int(body[:64], 16)
    stub = _pin_oracle_stub(_price_hex(price)) if price is not None else None
    ...
    elif oracle and stub and reference_return is not None:
        plan.add(OraclePin(
            oracle, stub, selector=oracle_selector,
            expected_return=reference_return,
            reference_block=block,
            reference_source=f"eth_call:{_block_tag(block)}",
        ), note=(f"blind f_orc candidate: {oracle[:10]} selector=0x{oracle_selector} "
                 "pinned at block-1 with exact historical return"))
    else:
        plan.notes.append(
            f"blind f_orc unsupported: oracle={oracle or 'N/A'} why={why_oracle}; "
            f"price={'known' if price is not None else 'missing'}; "
            f"reference_return={'known' if reference_return is not None else 'missing'}; "
