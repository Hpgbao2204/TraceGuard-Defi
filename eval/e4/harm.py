...
from core.harm.ledger import HarmSpecification, assess_ledger
...
def _finite_nonnegative(value: Any) -> float | None:
    """Parse a finite non-negative measurement; invalid values are unknown."""
    try:
        parsed = float(value)
...
def _loss_from_receipt_data(
    ...
    for log in receipt.get("logs") or []:
        ...
        net[token] += 0  # ensure token is represented even for internal moves
        if victims and sender in victims:
            net[token] -= amount
        if victims and recipient in victims:
            net[token] += amount
    ...
    for token, delta in net.items():
        ...
        if metadata is None:
            if delta:
                return None
            continue
        parsed = _price_metadata(metadata)
        if parsed is None:
            return None
        usd_per_token, decimals = parsed
        price = usd_per_token / (10 ** decimals)
        total += delta * price
        valued += 1
    if not valued:
        return None
...
def assess_harm(
    ...
    *,
    ...
    if baseline and disclosed_loss_usd is not None:
        disclosed = _finite_nonnegative(disclosed_loss_usd)
        if disclosed is None:
            return HarmAssessment("UNKNOWN", reason="invalid disclosed incident loss")
        if disclosed > 0:
            return HarmAssessment("HARM", disclosed, "disclosed_incident_loss")
    if not receipt:
    ...
    reference_block = spec.get("price_reference_block")
    target_block = spec.get("target_block")
    provenance = spec.get("price_provenance")
    if (not isinstance(reference_block, int) or reference_block < 0 or
            not isinstance(target_block, int) or target_block <= reference_block or
            not isinstance(provenance, dict)):
        return HarmAssessment("UNKNOWN", reason="price provenance missing or out of order")
    for token in prices:
        record = provenance.get(str(token).lower())
        if (not isinstance(record, dict) or
                record.get("reference_block") != reference_block or
                not isinstance(record.get("source"), str) or
                not record["source"]):
            return HarmAssessment("UNKNOWN", reason=f"price provenance missing for {token}")
    threshold = _finite_nonnegative(spec.get("lmin_usd", 100_000.0))
    if threshold is None:
        return HarmAssessment("UNKNOWN", reason="invalid Lmin USD threshold")
    ...
    for log in receipt.get("logs") or []:
        ...
        try:
            ...
            return HarmAssessment("UNKNOWN", reason="invalid transfer amount")
        token = str(log.get("address") or "").lower()
        if sender in victims:
            deltas.setdefault(sender, {})[token] = deltas.setdefault(sender, {}).get(token, 0) - amount
        if recipient in victims:
            deltas.setdefault(recipient, {})[token] = deltas.setdefault(recipient, {}).get(token, 0) + amount
    assets = {}
    for token, metadata in prices.items():
        parsed = _price_metadata(metadata)
        if parsed is None:
            return HarmAssessment("UNKNOWN", reason=f"invalid price metadata for {token}")
        usd_per_token, decimals = parsed
        assets[str(token).lower()] = (decimals, Decimal(str(usd_per_token)))
    ledger = assess_ledger(
        HarmSpecification(frozenset(victims), assets, Decimal(str(threshold)), measure="pool_debit"),
        deltas,
    )
    if ledger.status.value == "UNKNOWN":
        return HarmAssessment("UNKNOWN", reason=ledger.reason)
    loss = float(ledger.value or 0)
...
def _price_metadata(metadata: Any) -> tuple[float, int] | None:
    ...
    if isinstance(metadata, dict):
        if "usd_per_token" not in metadata:
            return None
        try:
            price = _finite_nonnegative(metadata["usd_per_token"])
            decimals = int(metadata.get("decimals", 18))
        except (TypeError, ValueError):
            return None
        if price is None or not 0 <= decimals <= 255:
            return None
        return price, decimals
    try:
        price = _finite_nonnegative(metadata)
        return (price, 18) if price is not None else None
    except (TypeError, ValueError):
        return None


def _validate_spec_price_provenance(
    spec: dict, required_keys: tuple[str, ...], *, target_block: int | None = None,
) -> str | None:
    """Validate explicit historical provenance for protected-harm prices."""
    reference_block = spec.get("price_reference_block")
    if not isinstance(reference_block, int) or reference_block < 0:
        return "price reference block missing or invalid"
    effective_target = spec.get("target_block", target_block)
    if not isinstance(effective_target, int) or effective_target <= reference_block:
...
def assess_attacker_value_harm(
    ...
    *,
    ...
    validated_native_price = None if native_price_usd is None else _finite_nonnegative(native_price_usd)
    if native_price_usd is not None and validated_native_price is None:
        return HarmAssessment("UNKNOWN", reason="invalid native USD price")
    validated_lmin = _finite_nonnegative(lmin_usd)
    if validated_lmin is None:
        return HarmAssessment("UNKNOWN", reason="invalid Lmin USD threshold")
    net_valued = 0.0
    ...
    for address, deltas in raw_deltas.items():
        for asset, delta in deltas.items():
            ...
            if asset == "native":
                if validated_native_price is None:
                    if delta > 0:
                        unpriced_positive.append(f"{address}:native")
                    continue
                net_valued += (delta / 10**18) * validated_native_price
                continue
    ...
    status = "HARM" if net_valued > validated_lmin else "NO_HARM"
    return HarmAssessment(
        status,
        net_valued,
        "attacker_value_delta",
        f"net_valued={net_valued:.2f}; deltas_by_address={deltas_by_address}; candidates={sorted(candidates)}; Lmin={validated_lmin}",
    )
...
def assess_pool_balance_delta(
    ...
    *,
    ...
    provenance_error = _validate_spec_price_provenance(
        spec, ("native_price_usd",), target_block=spec.get("target_block"))
    if provenance_error:
        return HarmAssessment("UNKNOWN", source="pool_balance_delta",
                              reason=provenance_error)
    try:
        price = _finite_nonnegative(spec["native_price_usd"])
        lmin = _finite_nonnegative(spec.get("lmin_usd", DEFAULT_LMIN_USD))
    except (KeyError, TypeError, ValueError, OverflowError):
        return HarmAssessment("UNKNOWN", source="pool_balance_delta",
                              reason="native price or Lmin is invalid")
    if price is None or lmin is None:
        return HarmAssessment("UNKNOWN", source="pool_balance_delta",
                              reason="native price or Lmin is invalid")
...
def assess_euler_bad_debt_delta(target: dict, harm_spec: dict | None) -> HarmAssessment:
    ...
    Amounts are raw token units scaled by the declared ``debt_decimals`` and
    ``collateral_decimals``; ``collateral_underlying_per_token`` is an
    underlying-token amount per whole eToken, frozen before replay.
    """
    spec = harm_spec or {}
    required = ("violator", "debt_token", "collateral_token",
                "collateral_underlying_per_token", "debt_price_usd",
                "collateral_price_usd", "debt_decimals", "collateral_decimals",
                "liquidation_event_address")
    missing = [key for key in required if key not in spec]
    if missing:
        return HarmAssessment("UNKNOWN", source="euler_bad_debt_delta",
                              reason=f"missing preregistered fields: {missing}")
    provenance_error = _validate_spec_price_provenance(
        spec, ("debt_price_usd", "collateral_price_usd"),
        target_block=spec.get("target_block"))
    if provenance_error:
        return HarmAssessment("UNKNOWN", source="euler_bad_debt_delta",
                              reason=provenance_error)
    rate_provenance = spec.get("collateral_rate_provenance")
    if not isinstance(rate_provenance, dict):
        return HarmAssessment("UNKNOWN", source="euler_bad_debt_delta",
                              reason="collateral exchange-rate provenance missing")
    rate_block = rate_provenance.get("reference_block")
    rate_source = rate_provenance.get("source")
    target_block = spec.get("target_block")
    if (not isinstance(rate_block, int) or rate_block < 0 or
            not isinstance(rate_source, str) or not rate_source.strip() or
            (isinstance(target_block, int) and rate_block >= target_block)):
        return HarmAssessment("UNKNOWN", source="euler_bad_debt_delta",
                              reason="collateral exchange-rate provenance invalid")
    try:
        violator = str(spec["violator"]).lower()
        debt_token = str(spec["debt_token"]).lower()
        collateral_token = str(spec["collateral_token"]).lower()
        debt_price = _finite_nonnegative(spec["debt_price_usd"])
        collateral_price = _finite_nonnegative(spec["collateral_price_usd"])
        collateral_rate = _finite_nonnegative(spec["collateral_underlying_per_token"])
        lmin = _finite_nonnegative(spec.get("lmin_usd", DEFAULT_LMIN_USD))
        debt_decimals = int(spec["debt_decimals"])
        collateral_decimals = int(spec["collateral_decimals"])
    ...
    if (not violator or not debt_token or not collateral_token or
            any(value is None for value in (debt_price, collateral_price,
                                             collateral_rate, lmin)) or
            not 0 <= debt_decimals <= 255 or
            not 0 <= collateral_decimals <= 255):
        return HarmAssessment("UNKNOWN", source="euler_bad_debt_delta",
    ...
    debt_usd = debt_after / 10**debt_decimals * debt_price
    collateral_usd = (collateral_after / 10**collateral_decimals *
                      collateral_rate * collateral_price)
...
class AttackerValueOracle:
    ...
    def assess(self, receipt, harm_spec, *, disclosed_loss_usd=None, baseline=False):
        ...
        return assess_attacker_value_harm(
            ...
            lmin_usd=spec.get("lmin_usd", DEFAULT_LMIN_USD),
        )
