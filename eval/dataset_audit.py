...
def audit(cache: Path) -> dict:
    ...
    return {
        ...
        "scientific_constraints": [
            ...
            (
                "Stage-1 state-delta coverage is "
                f"{coverage['attack'].get('state_delta', 0)}/"
                f"{coverage['attack'].get('call_structure', 0)} attacks and "
                f"{coverage['benign'].get('state_delta', 0)}/"
                f"{coverage['benign'].get('call_structure', 0)} background; "
                "missing state-delta observations remain unavailable."
            ),
            (
