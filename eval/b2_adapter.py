...
def run(context: str | Path, *, timeout: int = 300,
        ...
        target_code_copy: dict[str, str] | None = None,
        target_storage: dict[str, dict[str, str]] | None = None,
        target_index: int | None = None) -> B2Run:
    root = Path(context)
    required = (root / "block.json", root / "ancestors.json",
                root / "transactions.json", root / "receipts.json",
                root / "prestates.json", root / "poststates.json",
    ...
    for destination, source in (target_code_copy or {}).items():
        command.extend(["--target-code-copy", f"{destination}={source}"])
