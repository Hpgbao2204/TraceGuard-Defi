...
SEED = 42
TEST_FRAC = 0.20
VIEWS = SCREENING_VIEWS  # Canonical Stage 1 contract
require_screening_views(VIEWS)
...
def build_dataset(cache_path: Path) -> dict:
    ...
    + explicit unavailable delta when no transaction-scoped state observation
      exists; missing state-delta is represented by coverage, not fabrication.
    ...
    for h, r in cache.items():
        ...
        rows.append({
            "tx_hash": h,
            "label": label,
            "chain": r.get("chain") or r.get("network") or "mainnet",
            "block": r.get("block"),
            "incident_id": r.get("incident_id") or r.get("incident")
            or r.get("attack_id"),
