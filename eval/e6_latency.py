...
class Stage1Measurer:
    ...
    def __init__(self, client: RpcClient, cache: dict[str, float] | None = None):
        # E6 measures the explicitly synthetic demo screener unless a caller
        # supplies a model-bound screener through the benchmark harness.
        self.screener = Screener.demo(fetcher=TraceFetcher(client))
        self.cache = cache or {}
...
def print_summary(all_rows: list[dict], stage2_measured: bool) -> None:
    ...
    for (layer, metric), rs in sorted(groups.items()):
        ...
        if metric.endswith("_p50") or metric.endswith("_p95") or metric.endswith("_max"):
            ...
            display = "N/A" if val in (None, "", "N/A") else _fmt(float(val))
            print(f"  {layer:<10} {metric:<28} {display:>10} {row['unit']:<8} "
                  f"[{method}] {n}")
        else:
...
def main(argv: list[str] | None = None) -> int:
        ...
        if benign_hashes:
            if benign_lat:
                ...
                for metric in ("p50", "p95", "max"):
                    all_rows.append({"layer": "stage1", "metric": f"screener_benign_per_tx_{metric}",
                                     "value": "N/A", "unit": "ms", "method": "unavailable",
                                     "source": f"cache entries={len(benign_hashes)}; no recorded latency"})
    ...
    if args.measure_stage2:
        ...
        all_rows.append({"layer": "stage2", "metric": "rpc_calls_per_case",
                         ...
                         "source": "anvil lazy-fetch estimate; run with "
                                   "--measure-stage2 for measured stage-2 calls"})
    else:
