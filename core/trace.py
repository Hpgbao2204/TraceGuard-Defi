...
class TraceFetcher:
    ...
    def state_delta(self, tx_hash: str, block: int | None = None,
        ...
        # Block-boundary balance/nonce differences are not transaction-scoped:
        # other transactions in the same block would be attributed to tx_hash.
        # Do not fabricate a state delta when the transaction-level tracers are
        # unavailable; callers must treat this view as missing.
        return {
            "tx_hash": tx_hash, "block": blk, "pre_block": None,
            "method": "unavailable",
            "balances": {}, "nonces": {}, "storage": {},
            "accounts_checked": 0,
            "unavailable_reason": "transaction-scoped state tracer unavailable",
        }
...
def infer_state_delta_from_trace(trace: dict) -> dict:
    ...
    # parse_call_tracer guarantees flat_calls[0] is the root frame.  The root
    # transfer was already applied above, so only nested execution edges are
    # added here.
