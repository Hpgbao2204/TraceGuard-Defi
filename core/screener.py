...
class Screener:
    ...
    def __init__(self, fusion: LogisticFusion | None = None, tau: float | None = None,
        ...
        if fusion is None:
            raise ValueError(
                "Screener requires an explicit research model; use "
                "Screener.demo() only for synthetic demonstration"
            )
        self.fusion = fusion
        t = tau
        if t is None:
    ...
    @classmethod
    def demo(cls, **kwargs) -> "Screener":
        """Construct the explicitly synthetic demonstration screener."""
        return cls(fusion=fit_seed_default(), **kwargs)
...
def demo(argv: list[str] | None = None) -> int:
    ...
    screener = Screener.demo(fetcher=TraceFetcher(client))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--tx" in argv:
        i = argv.index("--tx")
        print("ERROR: --tx requires an explicit model artifact; use --demo for synthetic mode.", file=sys.stderr)
        return 1
    print(__doc__)
    return 0
