...
li; score l bn chun ha [0,1] theo proposal draft §3.1:
  ...
  s₄  economic signs  — 0.20 · (six explicit signals)
...
When an exact state-delta observation is unavailable, that feature dimension
is marked missing with an explicit coverage indicator; it is never fabricated
from block-boundary or inferred approximate data.
...
def token_flow_features(trace: dict) -> dict:
    ...
    calls = trace.get("flat_calls", [])
    for call in calls[1:] if calls else []:
...
def view_state_delta(delta: dict | None, trace: dict | None = None) -> dict:
    ...
    """
    f = state_delta_features(delta or {})
...
def economic_features(trace: dict) -> dict:
    ...
    - slippage      — observed zero minimum output for supported static ABI.
      Unsupported/dynamic layouts are marked unavailable.
    - raw_transfer_imbalance — diagnostic flag only: raw per-token transfer
      volume returning to the origin exceeds raw outgoing volume. Values are
      not comparable across tokens and are not economic profit.
    """
    calls = trace.get("flat_calls", [])
    logs = trace.get("logs", [])
    ...
    for c in calls:
        ...
        # swapExactTokensForTokens(addresses..., amountIn, amountOutMin,...)
        # has amountOutMin at ABI word 1. swapTokensForExactTokens has no
        # amountOutMin, so it is intentionally unsupported here.
        word_index = {"0x38ed1739": 1, "0x414bf389": 6}.get(selector)
        if word_index is None:
            continue
        start = 2 + 8 + word_index * 64
        word = inp[start:start + 64]
        if len(word) != 64:
            continue
        slippage = int(word, 16) == 0
        if slippage:
            break
    ...
    if root_from:
        ...
        raw_transfer_imbalance = int(profit_amt > 0 and profit_amt > out_amt)

    return {
        "flash_loan": int(bool(flash)), "oracle_read": int(bool(oracle)),
        "price_dev": price_dev, "slippage": slippage,
        "slippage_observed": int(slippage is not None),
        "raw_transfer_imbalance": raw_transfer_imbalance,
...
def view_economic(trace: dict, delta: dict | None = None) -> dict:
    ...
    6 signals (5 gc + auth_viol t invariant catalog §6.2):
      flash_loan, oracle_read, price_dev, slippage, raw_transfer_imbalance,
      auth_viol
    ...
    signals = [f["flash_loan"], f["oracle_read"], f["price_dev"],
               int(f["slippage"] is True), f["raw_transfer_imbalance"],
               f["auth_viol"]]
    # Execution trace analysis and verification
