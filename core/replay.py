...
class Replayer:
    ...
    # ---- warm-up mid-block ----
    def warmup(self, prior_hashes: list[str], gas_limit_multiplier: float = 1.5,
               deadline_seconds: float = 120.0) -> list[str]:
        ...
        for index, h in enumerate(prior_hashes):
            if _time.monotonic() >= deadline:
                failed.extend(prior_hashes[index:])
                self.warmup_failures.append({
                    "tx_hash": h, "observed": False, "status": None,
                    "reason": f"warm-up deadline exceeded ({deadline_seconds:.1f}s)",
                })
                break
            try:
                tx = self.tx_parts(h)
