...
def _operate_guard_runtime(shadow: str, selector: str) -> str:
    ...
    code += b"\x61\x00\x00\x57"
    # Copy and return the implementation's returndata byte-for-byte.
    code += b"\x3d\x60\x00\x60\x00\x3e\x3d\x60\x00\xf3"
...
def _operate_success_runtime(shadow: str, selector: str) -> str:
    """Return a selector guard that preserves all non-target call semantics.

    The generated runtime intentionally uses only small, explicit blocks:
    selector dispatch, a normal ``DELEGATECALL`` to ``shadow``, and separate
    returndata ``RETURN``/``REVERT`` blocks.  This keeps the target mutation
    surgical and makes the EVM behavior independently testable.
    """
    return _selector_forwarding_runtime(shadow, selector, b"\x00" * 31 + b"\x01")


def _oracle_forwarding_runtime(shadow: str, selector: str, return_data: str) -> str:
    """Return a selector override that forwards non-target calls losslessly."""
    if not isinstance(return_data, str) or not return_data.startswith("0x"):
        raise ValueError("oracle return data must be 0x-prefixed hex")
    body = return_data[2:]
    if len(body) % 2 or any(char not in "0123456789abcdefABCDEF" for char in body):
        raise ValueError("oracle return data must be even-length hex")
    return _selector_forwarding_runtime(shadow, selector, bytes.fromhex(body))


def _selector_forwarding_runtime(shadow: str, selector: str, target_data: bytes) -> str:
    """Build a selector override with byte-for-byte forwarding semantics."""
    shadow_int, sel_int = int(shadow, 16), int(selector, 16)
    if not 0 <= sel_int <= 0xFFFFFFFF:
        raise ValueError("selector must be four bytes")
    if len(target_data) > 0xFFFF:
        raise ValueError("target return data exceeds PUSH2 length limit")
    code = bytearray()

    # calldatacopy(0, 0, calldatasize())
    code += b"\x36" + _push(0, 1) + _push(0, 1) + b"\x37"
    # if (bytes4(msg.data[0:32]) == selector) jump to target block
    code += _push(0, 1) + b"\x35" + _push(0xe0, 1) + b"\x1c"
    code += _push(sel_int, 4) + b"\x14"
    target_jump = len(code)
    code += b"\x61\x00\x00\x57"

    # delegatecall(gas(), shadow, 0, calldatasize(), 0, 0)
    code += _push(0, 1) + _push(0, 1) + b"\x36" + _push(0, 1)
    code += _push(shadow_int, 20) + b"\x5a\xf4"
    failure_jump = len(code)
    # JUMPI takes the destination first and condition second; invert the
    # delegatecall result so only a failed call reaches the REVERT block.
    code += b"\x15" + b"\x61\x00\x00\x57"

    # Successful forwarding: return returndata byte-for-byte.
    success_dest = len(code)
    code += b"\x5b" + b"\x3d" + _push(0, 1) + _push(0, 1) + b"\x3e"
    code += b"\x3d" + _push(0, 1) + b"\xf3"

    # Failed forwarding: revert with returndata byte-for-byte.
    failed_dest = len(code)
    code += b"\x5b" + b"\x3d" + _push(0, 1) + _push(0, 1) + b"\x3e"
    code += b"\x3d" + _push(0, 1) + b"\xfd"

    # Target selector: return the exact declared bytes.
    target_dest = len(code)
    code += b"\x5b"
    for offset in range(0, len(target_data), 32):
        chunk = target_data[offset:offset + 32].ljust(32, b"\x00")
        code += _push(int.from_bytes(chunk, "big"), 32) + _push(offset, 2) + b"\x52"
    code += _push(len(target_data), 2 if len(target_data) > 0xFF else 1) + _push(0, 1) + b"\xf3"

    code[target_jump + 1:target_jump + 3] = target_dest.to_bytes(2, "big")
    code[failure_jump + 2:failure_jump + 4] = failed_dest.to_bytes(2, "big")
    # Successful delegatecall falls through to success_dest; the leading JUMPDEST
    # is reached by the fall-through path and is therefore intentional.
    return "0x" + code.hex()
...
class OraclePin(Mutation):
    ...
    def __init__(self, oracle: str, stub_bytecode: str,
        ...
        self.reference_verified = bool(
            expected_return
            and reference_block is not None
            and reference_source.startswith("eth_call:")
        )
...
class ERC1271ValidationSubstitution(Mutation):
    """E5 operator for a proof-bound ERC-1271 validation call.

    The geth-replay backend performs the pre-dispatch return-data substitution.
    This object only constructs the exact descriptor; it never guesses a
    caller, depth, occurrence, or target address.
    """

    name = "e5_erc1271_validation"
    mutation_kind = "return-data-substitution"
    causal_ready = False
    target = "erc1271_validation_result"

    def __init__(self, caller: str, callee: str, depth: int, *,
                 selector: str = "0x1626ba7e", occurrence: int = 1):
        self.caller = caller.lower()
        self.callee = callee.lower()
        self.depth = int(depth)
        self.selector = selector.lower()
        self.occurrence = int(occurrence)
        if len(self.selector) != 10 or not self.selector.startswith("0x"):
            raise ValueError("ERC1271 selector must be a 4-byte 0x-prefixed value")
        if self.depth < 0 or self.occurrence < 1:
            raise ValueError("depth and occurrence must be non-negative/positive")

    def apply(self, fork: ForkRunner) -> None:
        raise RuntimeError("ERC1271ValidationSubstitution requires the geth replay backend")

    def descriptor(self, *, output: str, mode: str) -> dict:
        if mode not in {"sham", "counterfactual"}:
            raise ValueError("mode must be sham or counterfactual")
        if not isinstance(output, str) or not output.startswith("0x") or len(output) % 2:
            raise ValueError("output must be even-length 0x-prefixed hex")
        return {
            "intervention_type": "STATICCALL",
            "intervention_action": "substitute",
            "intervention_caller": self.caller,
            "intervention_callee": self.callee,
            "intervention_selector": self.selector,
            "intervention_depth": self.depth,
            "intervention_occurrence": self.occurrence,
...
class StorageSlotWrite(Mutation):
    """Explicit single-slot write for evidence-bound composite mutations.

    This primitive is intentionally generic but never discovers or authorizes
    a slot. Callers must provide a 32-byte address, slot, and value; coupled
    replay must supply the proof-bound authorization separately.
    """

    name = "storage_slot_write"
    mutation_kind = "insertion"
    causal_ready = False

    def __init__(self, address: str, slot: str, value: str,
                 *, provenance: str | None = None):
        self.address = address.lower()
        self.slot = slot.lower()
        self.value = value.lower()
        self.provenance = provenance
        self.validity_note = "requires proof-bound slot resolution and coupled authorization"
        if not self.address.startswith("0x") or len(self.address) != 42:
            raise ValueError("storage write address must be 20 bytes")
        for label, item in (("slot", self.slot), ("value", self.value)):
            if not item.startswith("0x") or len(item) != 66:
                raise ValueError(f"storage write {label} must be 32 bytes")
            int(item[2:], 16)
        if not provenance or not provenance.strip():
            raise ValueError("storage write provenance is required")

    def apply(self, fork: ForkRunner) -> None:
        RpcClient(fork.url, timeout=30).anvil_set_storage(
            self.address, self.slot, self.value,
        )
...
class ReentrancyGuardSubstitute(Mutation):
    """E5 auth-free reentrancy intervention with an explicit runtime input.

    Runtime synthesis is intentionally not guessed.  The caller must provide
    a frozen, source-faithful runtime artifact and an exact selector/frame
    descriptor.  Missing artifacts fail closed before any fork mutation.
    """

    name = "e5_reentrancy_guard"
    mutation_kind = "insertion"

    def __init__(self, target: str, patched_runtime: str | None = None,
                 selector: str | None = None, descriptor: dict | None = None):
        self.target = target.lower()
        self.patched_runtime = patched_runtime
        self.selector = selector.lower() if selector else None
        self.descriptor = descriptor or {}
        self.validity_note = "requires frozen source-faithful guard runtime and exact seam descriptor"

    def apply(self, fork: ForkRunner) -> None:
        if not self.patched_runtime:
            raise ValueError("E5_REENTRANCY_RUNTIME_REQUIRED")
        if not self.selector or self.descriptor.get("match_count") != 1:
            raise ValueError("E5_REENTRANCY_SEAM_NOT_UNIQUE")
        RpcClient(fork.url, timeout=30).anvil_set_code(self.target, self.patched_runtime)


class ArithmeticGuardInsert(Mutation):
    """E5 arithmetic guard intervention requiring a frozen runtime artifact."""

    name = "e5_arithmetic_guard"
    mutation_kind = "insertion"

    def __init__(self, target: str, selector: str, bound_expr: str,
                 patched_runtime: str | None = None, *, descriptor: dict | None = None):
        self.target = target.lower()
        self.selector = selector.lower()
        self.bound_expr = bound_expr
        self.patched_runtime = patched_runtime
        self.descriptor = descriptor or {}
        self.validity_note = "requires frozen patched runtime, bound expression, and unique selector seam"

    def apply(self, fork: ForkRunner) -> None:
        if not self.patched_runtime:
            raise ValueError("E5_ARITHMETIC_RUNTIME_REQUIRED")
        if self.descriptor.get("match_count") != 1:
            raise ValueError("E5_ARITHMETIC_SEAM_NOT_UNIQUE")
        if not self.bound_expr.strip():
            raise ValueError("E5_ARITHMETIC_BOUND_REQUIRED")
        RpcClient(fork.url, timeout=30).anvil_set_code(self.target, self.patched_runtime)
